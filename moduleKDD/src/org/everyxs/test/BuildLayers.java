...
public class BuildLayers {
    ...
    void build(GraphModel graphModel) {
        ...
        for (int i=0; i<newGraph.getNodeCount(); i++){
            ...
            if (layer == 0) {
                Edge e0 = graphModel.factory().newEdge(s, s, (float) (newGraph.getDegree(s)*1.0-1), false);
                newGraph.addEdge(e0);
            }
            else {
                Edge e0 = graphModel.factory().newEdge(s, s, (float) (float) 1/60, false);
                newGraph.addEdge(e0);
            }
                    ...
                    if (layer2 - layer == 1) {
                        ...
                        if (isDirected) {
                            ...
                            for (Edge e1: newGraph.getEdges(t))
                                degree += e1.getWeight();
                            Edge e = graphModel.factory().newEdge(s, t, (float) (degree/30), true);
                            ...
                            for (Edge e1: newGraph.getEdges(t))
                                degree += e1.getWeight();
                            Edge e = graphModel.factory().newEdge(s, t, (float) (degree/30), false);
