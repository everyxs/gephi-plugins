...
public class Replicator extends DynamicOperator {
    ...
    public Replicator(HierarchicalGraph g) throws NotConvergedException {
        super(g);
        dense = new double[size][size];
        for (int i=0; i<size; i++) {
            for (int j=0; j<size; j++){
                dense[i][j] = adjMatrix[i][j];
            }
        }
        DenseMatrix A = new DenseMatrix(dense);
        ...
        for (int i=0; i<size; i++) {
            reweight[i] = Math.abs(Pi.get(i, maxID)); //eigenvector centrality for replicator reweighting 
    ...
    public void execute(AttributeModel attributeModel) throws NotConvergedException {
        ...
        double[][] repMatrix = reweight(dense);
