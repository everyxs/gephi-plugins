...
class LocalPartition implements org.gephi.statistics.spi.Statistics, org.gephi.utils.longtask.spi.LongTask {
    ...
    public void execute(GraphModel gm, AttributeModel am) {
        ...
        Replicator operator; 
        try {
            operator = new Replicator(graph); //R operator
        ...
        for (int i=0; i<N; i++) {
            Node u = indicies.get(i);
            double tmp = Math.round(2*graph.getDegree(u)/operator0.scale[0]/1/1); //decay =1, quality bound=1 (needs a better GUI for parameter input)
            if (tmp <t)
                t = tmp;
        }
        ...
        for (int i = 0; i < N; i++) {
            ...
            list[i] = new NodeCompare(invIndicies.get(s), centralVector.get(i)/Math.sqrt(operator0.scale[0])); //scale needs to be an array with index i
            //Test code
            AttributeRow row = (AttributeRow) s.getNodeData().getAttributes();
            row.setValue(centrality, centralVector.get(i)/Math.sqrt(operator0.scale[0]));
            if (isCanceled) {
                return;
            }
        }
